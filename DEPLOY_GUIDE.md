...
您在本地启动服务后，生成的链接（如 `http://localhost:3000`）只能在您自己的电脑上访问。要与他人分享，您需要将服务暴露到公共互联网上。这里介绍两种常用方法：

### 方法一：使用内网穿透工具（适用于临时分享）

内网穿透工具可以为您本地运行的服务生成一个临时的公共 URL。**ngrok** 是一个非常流行的选择。
